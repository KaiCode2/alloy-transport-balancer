...
// Load-balance across multiple RPC providers
let transport = LoadBalancedTransport::new(vec![
    (Url::parse("https://eth-mainnet.g.alchemy.com/v2/YOUR_KEY").unwrap(), Weight(100)),
    (Url::parse("https://lb.drpc.org/ogrpc?network=ethereum").unwrap(), Weight(50)),
]);

// Use with alloy (requires alloy-rpc-client):
//   let client = alloy_rpc_client::RpcClient::new(transport, false);
...
let transport = LoadBalancedTransport::new(vec![
    (Url::parse("https://primary-provider.com").unwrap(), Weight(100)),   // 2x traffic
    (Url::parse("https://secondary-provider.com").unwrap(), Weight(50)),  // 1x traffic
    (Url::parse("https://fallback-provider.com").unwrap(), Weight(10)),   // backup only
]);
```
